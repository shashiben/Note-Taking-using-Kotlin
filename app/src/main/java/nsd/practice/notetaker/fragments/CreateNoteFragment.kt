...
import nsd.practice.notetaker.utils.NoteBottomSheetFragment
...
class CreateNoteFragment : BaseFragment(), EasyPermissions.PermissionCallbacks,
        ...
        if (noteId != -1) {
            ...
            launch {
                context?.let {
                    ...
                    subtitleField.setText(notes.subTitle)
                    noteDesc.setText(notes.noteText)
                    if (notes.imgPath != "" && notes.imgPath != null) {
    ...
    private fun updateNote() {
        launch {
            ...
            context?.let {
                ...
                notes.title = titleField.text.toString()
                notes.subTitle = subtitleField.text.toString()
    ...
    private fun deleteNote() {
        ...
        launch {
            context?.let {
                NotesDatabase.getDatabase(it).noteDao().deleteNoteByID(noteId)
                requireActivity().supportFragmentManager.popBackStack()
            }
        }
    }
    ...
    private fun saveNote() {
        when {
            titleField.text.trim().isEmpty() -> {
                Toast.makeText(context, "Please enter title", Toast.LENGTH_SHORT).show()
            }
            subtitleField.text.trim().isEmpty() -> {
